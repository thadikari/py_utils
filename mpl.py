...
def bind_plotting_args(parser):
    ...
    parser.add_argument('--backend', choices=matplotlib.rcsetup.all_backends, default=None)
    parser.add_argument('--ax_size', help='width, height {scale} per axis', type=float, nargs='+', action=AxSizeAction,
                        default=[8, 5])

    # plot_improvement args
    ...
    # plot_improvement args
    parser.add_argument('--plot_separate', help='plot axes on grid or separately', action='store_true')
    parser.add_argument('--share_x', action='store_true')
    parser.add_argument('--legend_ax', type=int, default=-1)
    parser.add_argument('--rows_cols', nargs=2, type=int, default=None)
    parser.add_argument('--hspace', help='hspace in subplots_adjust', type=float, default=0)
...
def plot_improvement(args, ax):
    ...
    if not args.imprv_percentage is None:
        for xval, bot, top, sgn in args.imprv_percentage:
            ...
            ax.annotate(f'{imprv}%', xy=(xval + np.sign(sgn) * 0.2, (top + bot) / 2), ha=ha, va='center',
                        size=args.font_size_imprv)
...
def set_best_time_scale(ax, seconds, label=''):
    if seconds > 3600 * 24 * 5:
        div, unit, steps = 3600 * 24, 'days', [8.64]
    elif seconds > 3600 * 5:
        div, unit, steps = 3600, 'hrs', [9]
    elif seconds > 60 * 5:
        div, unit, steps = 60, 'min', [3, 6, 9]
    elif seconds > 1 * 1:
        div, unit, steps = 1, 's', [1, 2, 3, 4, 5, 10]
    else:
        div, unit, steps = 1e-3, 'ms', [1, 2, 4, 5, 10]
...
def plot_callables(args, callables, title):
    if not args.plot_separate:
        plot_on_grid(args, callables, title)
    else:
        plot_separate(args, callables)


def plot_on_grid(args, callables, title):
    """
    Prep a grid of axes and call each item in `callable_list` to plot on the axis.
    """
    subplots_kwargs = {'share_x': 'col'} if args.share_x else {}
    axes, fig = get_subplot_axes(args.ax_size, len(callables),
                                 rows_cols=args.rows_cols,
                                 subplots_kwargs=subplots_kwargs)
    for ax, cb in zip(axes, callables): cb(ax)
    for ind, ax in enumerate(axes):
        if 0 <= args.legend_ax != ind:
            leg = ax.get_legend()
            if leg is not None:
                leg.remove()
        # set x label to only the bottom-most axes
        # following hardcoded line need to be refactored
        # if args.share_x and ind != 2: ax.set_xlabel(None)
    fig.canvas.manager.set_window_title(f'{title}: {args.data_dir}')
    tight = True
    if args.hspace > 0:
        plt.subplots_adjust(hspace=args.hspace)
        tight = False
    save_show_fig(args, plt, os.path.join(args.data_dir, title), tight_layout=tight)


def plot_separate(args, callables):
    """
    Call each item in `callable_list` to plot on the axis while saving figures.
    """
    fig = plt.gcf()
    for cb in callables:
        fig.clf()
        fig.set_size_inches(args.ax_size[0], args.ax_size[1])
        cb(plt.gca())
        title = cb.__name__
        fig.canvas.manager.set_window_title(f'{title}: {args.data_dir}')
        save_show_fig(args, plt, os.path.join(args.data_dir, title))


def get_fmt(ax):
    """
    Get format of next item in prop_cycler and advance one position.
    """
    prop = next(ax._get_lines.prop_cycler)
    return '%s%s' % (prop['color'], prop['linestyle'])
