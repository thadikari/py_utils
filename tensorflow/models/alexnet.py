...
def create_plh(with_data=True):
    ...
    if with_data:
        image = tf.compat.v1.placeholder(tf.float32, shape=[None, 227, 227, 3])
        label = tf.compat.v1.placeholder(tf.float32, shape=[None])
...
def create_model(images, keep_prob):
    ...
    layers = tf.layers
    vv = lambda shape: tf.Variable(lambda: tf.random.truncated_normal(shape=shape, mean=0, stddev=0.08))

    # Convolution Layer 1 | Response Normalization | Max Pooling | ReLU
    c_layer_1 = tf.nn.conv2d(input=images, filters=vv([11,11,3,96]), strides=[1, 4, 4, 1], padding="VALID", name="c_layer_1")
    ...
    c_layer_1 = tf.nn.max_pool2d(input=c_layer_1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="VALID")

    # Convolution Layer 2 | Response Normalization | Max Pooling | ReLU
    c_layer_2 = tf.nn.conv2d(input=c_layer_1, filters=vv([5,5,96,256]), strides=[1, 1, 1, 1], padding="SAME", name="c_layer_2")
    c_layer_2 = tf.nn.relu(c_layer_2)
    # c_layer_2 = tf.nn.lrn(c_layer_2, depth_radius=5, bias=K_BIAS, alpha=ALPHA, beta=BETA)
    c_layer_2 = tf.nn.max_pool2d(input=c_layer_2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="VALID")

    # Convolution Layer 3 | ReLU
    c_layer_3 = tf.nn.conv2d(input=c_layer_2, filters=vv([3,3,256,384]), strides=[1, 1, 1, 1], padding="SAME", name="c_layer_3")
    c_layer_3 = tf.nn.relu(c_layer_3)

    # Convolution Layer 4 | ReLU
    c_layer_4 = tf.nn.conv2d(input=c_layer_3, filters=vv([3,3,384,384]), strides=[1, 1, 1, 1], padding="SAME", name="c_layer_4")
    c_layer_4 = tf.nn.relu(c_layer_4)

    # Convolution Layer 5 | ReLU | Max Pooling
    c_layer_5 = tf.nn.conv2d(input=c_layer_4, filters=vv([3,3,384,256]), strides=[1, 1, 1, 1], padding="SAME", name="c_layer_5")
    c_layer_5 = tf.nn.relu(c_layer_5)
    c_layer_5 = tf.nn.max_pool2d(input=c_layer_5, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="VALID")
    ...
    fc_layer_1 = tf.nn.dropout(fc_layer_1, rate=1 - (keep_prob))

    # Fully Connected Layer 2 | Dropout
    fc_layer_2 = fc(inputs=fc_layer_1, num_outputs=4096, activation_fn=tf.nn.relu)
    fc_layer_2 = tf.nn.dropout(fc_layer_2, rate=1 - (keep_prob))
